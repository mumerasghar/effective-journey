...
class Encoder(tf.keras.layers.Layer):
    def __init__(self, num_layers, d_model, num_heads, dff, rate=0.1):
        ...
        for _ in range(num_layers):
            if _ == 0:
                self.s_attention.append(SelfAttention(
                    dff, d_model, convert_dim=True))
            else:
        ...
        self.conv_net = tf.keras.Sequential([
            tf.keras.layers.Conv2D(
                d_model, (3, 3), padding='same', activation='relu')
        ])
...
class Transformer(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, target_vocab_size, max_pos_encoding,
        ...
        self.decoder = Decoder(
            num_layers, d_model, num_heads, dff, target_vocab_size, max_pos_encoding, rate)
...
def checkpoint_manager(checkpoint_dir='checkpoints'):
    ...
    ckpt_manager = tf.train.CheckpointManager(
        checkpoint, checkpoint_dir, max_to_keep=30)
...
def initialize_model():
    ...
    for (batch, (img_tensor, tar, img_name)) in enumerate(dataset.take(1)):
        print(img_name)
        tar_inp = tar[:, :-1]
        tar_real = tar[:, 1:]
        ...
        transformer(img_tensor, tar_inp, True, dec_mask)
...
def main(epochs, o_break=False, transfer_learning=True):
    ...
    if transfer_learning:
        ...
        transformer1.layers[0].set_weights(pre_trained_encoder_weights)
        transformer1.layers[0].trainable = False

        transformer = transformer1
        del transformer1
        del pre_trained_encoder_weights
...
params1 = {
    ...
    "dff": cfg1['DFF'],
    "target_vocab_size": cfg1['VOCAB_SIZE'],
    "max_pos_encoding": cfg1['VOCAB_SIZE'],
    "rate": cfg1['DROP_RATE']
}

transformer = Transformer(**params1)
transformer1 = Transformer(**params)
