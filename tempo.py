...
i_data = i_data.map(
    lambda item1, item2: tf.numpy_function(v_map_func, [item1, item2], [tf.float32, tf.int32, tf.string, tf.float32]),
    num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def main(epochs, o_break=False):
    ...
    for epoch in range(epochs):
        ...
        transformer.save_weights(f'checkpoints/transformer_prev_{epoch}')
